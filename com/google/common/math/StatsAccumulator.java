	...
	public void add(double value) {
		if (this.count == 0L) {
			...
			if (Doubles.isFinite(value) && Doubles.isFinite(this.mean)) {
				...
				this.mean = this.mean + delta / (double)this.count;
		...
		if (values.count() != 0L) {
			if (this.count == 0L) {
				...
				if (Doubles.isFinite(this.mean) && Doubles.isFinite(values.mean())) {
					...
					this.mean = this.mean + delta * (double)values.count() / (double)this.count;
					this.sumOfSquaresOfDeltas = this.sumOfSquaresOfDeltas + values.sumOfSquaresOfDeltas() + delta * (values.mean() - this.mean) * (double)values.count();
				} else {
