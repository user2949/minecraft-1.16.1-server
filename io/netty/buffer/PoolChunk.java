...
final class PoolChunk<T> implements PoolChunkMetric {
	...
	private long allocateRun(int normCapacity) {
		...
		if (id < 0) {
			...
			this.freeBytes = this.freeBytes - this.runLength(id);
			return (long)id;
		}
	}
	...
	void free(long handle) {
		...
		this.freeBytes = this.freeBytes + this.runLength(memoryMapIdx);
	...
	private void initBufWithSubpage(PooledByteBuf<T> buf, long handle, int bitmapIdx, int reqCapacity) {
		...
		buf.init(
			this,
			handle,
			this.runOffset(memoryMapIdx) + (bitmapIdx & 1073741823) * subpage.elemSize + this.offset,
			reqCapacity,
			subpage.elemSize,
