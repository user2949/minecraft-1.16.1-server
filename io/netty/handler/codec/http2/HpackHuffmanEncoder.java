...
final class HpackHuffmanEncoder {
	...
	private final HpackHuffmanEncoder.EncodedLengthProcessor encodedLengthProcessor = new HpackHuffmanEncoder.EncodedLengthProcessor();
	private final HpackHuffmanEncoder.EncodeProcessor encodeProcessor = new HpackHuffmanEncoder.EncodeProcessor();
	...
	private final class EncodeProcessor implements ByteProcessor {
		...
		public boolean process(byte value) {
			...
			this.current <<= nbits;
			this.current = this.current | (long)HpackHuffmanEncoder.this.codes[b];
			this.n += nbits;
		...
		void end() {
			try {
				if (this.n > 0) {
					...
					this.current = this.current | (long)(255 >>> this.n);
	...
	private final class EncodedLengthProcessor implements ByteProcessor {
		...
		@Override
		public boolean process(byte value) {
			this.len = this.len + (long)HpackHuffmanEncoder.this.lengths[value & 255];
			return true;
		}
