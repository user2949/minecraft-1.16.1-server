...
public class InboundHttp2ToHttpAdapter extends Http2EventAdapter {
	...
	protected final void removeMessage(Http2Stream stream, boolean release) {
		FullHttpMessage msg = stream.removeProperty(this.messageKey);
		if (release && msg != null) {
			msg.release();
		}
	}
	...
	protected FullHttpMessage newMessage(Http2Stream stream, Http2Headers headers, boolean validateHttpHeaders, ByteBufAllocator alloc) throws Http2Exception {
		return (FullHttpMessage)(this.connection.isServer()
	...
	protected FullHttpMessage processHeadersBegin(
		ChannelHandlerContext ctx, Http2Stream stream, Http2Headers headers, boolean endOfStream, boolean allowAppend, boolean appendToTrailer
	...
	public void onHeadersRead(
		...
		if (msg != null) {
			if (streamDependency != 0) {
				msg.headers().setInt(ExtensionHeaderNames.STREAM_DEPENDENCY_ID.text(), streamDependency);
			}

			msg.headers().setShort(ExtensionHeaderNames.STREAM_WEIGHT.text(), weight);
			this.processHeadersEnd(ctx, stream, msg, endOfStream);
		}
	}
	...
	public void onPushPromiseRead(ChannelHandlerContext ctx, int streamId, int promisedStreamId, Http2Headers headers, int padding) throws Http2Exception {
		...
		if (msg == null) {
			...
			msg.headers().setShort(ExtensionHeaderNames.STREAM_WEIGHT.text(), (short)16);
