...
final class FastLz {
		...
		if (inLength < 4) {
			...
			while (ip < ipLimit) {
				...
				if (--distance == 0L) {
					...
					while (ip < ipBound && input[inOffset + ref++] == x) {
						ip++;
					}
				...
				int hvalx = hashFunction(input, inOffset + ip);
				htab[hvalx] = ip++;
				hvalx = hashFunction(input, inOffset + ip);
				htab[hvalx] = ip++;
				output[outOffset + op++] = 31;
			}
			...
			do {
				...
				if (ctrl >= 32L) {
					...
					if (level == 1) {
						...
						int code = input[inOffset + ip++] & 255;
						ref -= code;
	...
	private static int readU16(byte[] data, int offset) {
		return offset + 1 >= data.length ? data[offset] & 0xFF : (data[offset + 1] & 0xFF) << 8 | data[offset] & 0xFF;
	}
