...
public class SslHandler extends ByteToMessageDecoder implements ChannelOutboundHandler {
		...
		try {
			...
			while (!ctx.isRemoved()) {
				...
				if (result.getStatus() == Status.CLOSED) {
					...
					promise = null;
					this.pendingUnencryptedWrites.releaseAndFailAll(ctx, SSLENGINE_CLOSED);
					return;
				}
							...
							if (readableBytes == 0 && previousOverflowReadableBytes == 0) {
								throw new IllegalStateException(
									"Two consecutive overflows but no content was consumed. "
										...
										+ this.engine.getSession().getApplicationBufferSize()
										+ " maybe too small."
								);
							}
	...
	private void handshake(Promise<Channel> newHandshakePromise) {
		...
		try {
			...
			this.setHandshakeFailure(ctx, var8);
		} finally {
			this.forceFlush(ctx);
		}
	...
	private static enum SslEngineType {
		...
		abstract SSLEngineResult unwrap(SslHandler sslHandler, ByteBuf byteBuf2, int integer3, int integer4, ByteBuf byteBuf5) throws SSLException;

		abstract int calculateWrapBufferCapacity(SslHandler sslHandler, int integer2, int integer3);

		abstract int calculatePendingData(SslHandler sslHandler, int integer);
