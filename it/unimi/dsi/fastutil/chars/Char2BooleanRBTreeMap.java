...
public class Char2BooleanRBTreeMap extends AbstractChar2BooleanSortedMap implements Serializable, Cloneable {
				...
				while (true) {
					...
					if (!this.dirPath[i - 1]) {
						...
						if (this.nodePath[i - 1].succ() || y.black()) {
							...
							Char2BooleanRBTreeMap.Entry xx = this.nodePath[i - 1];
							xx.black(false);
							y.black(true);
							xx.left = y.right;
							y.right = xx;
						...
						if (this.nodePath[i - 1].pred() || y.black()) {
							...
							Char2BooleanRBTreeMap.Entry xxx = this.nodePath[i - 1];
							xxx.black(false);
							y.black(true);
							xxx.right = y.left;
							y.left = xxx;
			...
			if (p.black()) {
				for (; i > 0; i--) {
					...
					if (!this.dirPath[i - 1]) {
						...
						Char2BooleanRBTreeMap.Entry wx = this.nodePath[i - 1].left;
						if (!wx.black()) {
							wx.black(true);
							this.nodePath[i - 1].black(false);
						...
						if (!wx.pred() && !wx.left.black() || !wx.succ() && !wx.right.black()) {
							if (wx.pred() || wx.left.black()) {
								Char2BooleanRBTreeMap.Entry y = wx.right;
								y.black(true);
								wx.black(false);
								wx.right = y.left;
								y.left = wx;
	...
	public ObjectSortedSet<it.unimi.dsi.fastutil.chars.Char2BooleanMap.Entry> char2BooleanEntrySet() {
		if (this.entries == null) {
			this.entries = new AbstractObjectSortedSet<it.unimi.dsi.fastutil.chars.Char2BooleanMap.Entry>() {
				final Comparator<? super it.unimi.dsi.fastutil.chars.Char2BooleanMap.Entry> comparator = (x, y) -> Char2BooleanRBTreeMap.this.actualComparator
	...
	private class EntryIterator extends Char2BooleanRBTreeMap.TreeIterator implements ObjectListIterator<it.unimi.dsi.fastutil.chars.Char2BooleanMap.Entry> {
		EntryIterator() {
		}
	...
	private final class Submap extends AbstractChar2BooleanSortedMap implements Serializable {
		...
		public boolean get(char k) {
			Char2BooleanRBTreeMap.Entry e;
			return this.in(k) && (e = Char2BooleanRBTreeMap.this.findKey(k)) != null ? e.value : this.defRetValue;
		}
		...
		private class SubmapEntryIterator
			extends Char2BooleanRBTreeMap.Submap.SubmapIterator
