...
public class Char2FloatOpenHashMap extends AbstractChar2FloatMap implements Serializable, Cloneable, Hash {
			...
			this.values = new AbstractFloatCollection() {
				...
				public void forEach(DoubleConsumer consumer) {
					if (Char2FloatOpenHashMap.this.containsNullKey) {
						consumer.accept((double)Char2FloatOpenHashMap.this.value[Char2FloatOpenHashMap.this.n]);
					}
					...
					while (pos-- != 0) {
						if (Char2FloatOpenHashMap.this.key[pos] != 0) {
							consumer.accept((double)Char2FloatOpenHashMap.this.value[pos]);
						}
					}
				}
			};
	...
	private void writeObject(ObjectOutputStream s) throws IOException {
		char[] key = this.key;
		float[] value = this.value;
		Char2FloatOpenHashMap.MapIterator i = new Char2FloatOpenHashMap.MapIterator();
	...
	private class EntryIterator extends Char2FloatOpenHashMap.MapIterator implements ObjectIterator<it.unimi.dsi.fastutil.chars.Char2FloatMap.Entry> {
	...
	private class FastEntryIterator extends Char2FloatOpenHashMap.MapIterator implements ObjectIterator<it.unimi.dsi.fastutil.chars.Char2FloatMap.Entry> {
	...
	final class MapEntry implements it.unimi.dsi.fastutil.chars.Char2FloatMap.Entry, java.util.Map.Entry<Character, Float> {
		...
		public boolean equals(Object o) {
			if (!(o instanceof java.util.Map.Entry)) {
				...
				return Char2FloatOpenHashMap.this.key[this.index] == (Character)e.getKey()
					&& Float.floatToIntBits(Char2FloatOpenHashMap.this.value[this.index]) == Float.floatToIntBits((Float)e.getValue());
			}
		}
	...
	private final class MapEntrySet extends AbstractObjectSet<it.unimi.dsi.fastutil.chars.Char2FloatMap.Entry> implements FastEntrySet {
		...
		public void forEach(Consumer<? super it.unimi.dsi.fastutil.chars.Char2FloatMap.Entry> consumer) {
			if (Char2FloatOpenHashMap.this.containsNullKey) {
				consumer.accept(
					new BasicEntry(Char2FloatOpenHashMap.this.key[Char2FloatOpenHashMap.this.n], Char2FloatOpenHashMap.this.value[Char2FloatOpenHashMap.this.n])
				);
			}
			...
			while (pos-- != 0) {
				if (Char2FloatOpenHashMap.this.key[pos] != 0) {
					consumer.accept(new BasicEntry(Char2FloatOpenHashMap.this.key[pos], Char2FloatOpenHashMap.this.value[pos]));
				}
			}
		}
