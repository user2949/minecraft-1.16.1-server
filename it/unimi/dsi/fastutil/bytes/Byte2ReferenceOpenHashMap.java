...
public class Byte2ReferenceOpenHashMap<V> extends AbstractByte2ReferenceMap<V> implements Serializable, Cloneable, Hash {
	...
	private void writeObject(ObjectOutputStream s) throws IOException {
		byte[] key = this.key;
		V[] value = this.value;
		Byte2ReferenceOpenHashMap<V>.MapIterator i = new Byte2ReferenceOpenHashMap.MapIterator();
	...
	private class EntryIterator extends Byte2ReferenceOpenHashMap<V>.MapIterator implements ObjectIterator<it.unimi.dsi.fastutil.bytes.Byte2ReferenceMap.Entry<V>> {
	...
	private class FastEntryIterator
		extends Byte2ReferenceOpenHashMap<V>.MapIterator
	...
	private final class MapEntrySet extends AbstractObjectSet<it.unimi.dsi.fastutil.bytes.Byte2ReferenceMap.Entry<V>> implements FastEntrySet<V> {
		...
		public void forEach(Consumer<? super it.unimi.dsi.fastutil.bytes.Byte2ReferenceMap.Entry<V>> consumer) {
			if (Byte2ReferenceOpenHashMap.this.containsNullKey) {
				consumer.accept(
					new BasicEntry(
						Byte2ReferenceOpenHashMap.this.key[Byte2ReferenceOpenHashMap.this.n], Byte2ReferenceOpenHashMap.this.value[Byte2ReferenceOpenHashMap.this.n]
					)
				);
			}
			...
			while (pos-- != 0) {
				if (Byte2ReferenceOpenHashMap.this.key[pos] != 0) {
					consumer.accept(new BasicEntry(Byte2ReferenceOpenHashMap.this.key[pos], Byte2ReferenceOpenHashMap.this.value[pos]));
				}
			}
		}
