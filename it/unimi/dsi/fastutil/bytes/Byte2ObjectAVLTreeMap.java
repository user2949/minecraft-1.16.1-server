...
public class Byte2ObjectAVLTreeMap<V> extends AbstractByte2ObjectSortedMap<V> implements Serializable, Cloneable {
	...
	private Byte2ObjectAVLTreeMap.Entry<V> parent(Byte2ObjectAVLTreeMap.Entry<V> e) {
		if (e == this.tree) {
			...
			Byte2ObjectAVLTreeMap.Entry<V> x;
		...
		if (this.tree == null) {
			...
			while (q != null) {
				...
				if (!dir) {
					...
					if (y.balance() == -2) {
						Byte2ObjectAVLTreeMap.Entry<V> xx = y.left;
						...
						if (xx.balance() == 1) {
							...
							Byte2ObjectAVLTreeMap.Entry<V> wx = xx.right;
							xx.right = wx.left;
							wx.left = xx;
							y.left = wx.right;
							wx.right = y;
	...
	public ObjectSortedSet<it.unimi.dsi.fastutil.bytes.Byte2ObjectMap.Entry<V>> byte2ObjectEntrySet() {
		if (this.entries == null) {
			this.entries = new AbstractObjectSortedSet<it.unimi.dsi.fastutil.bytes.Byte2ObjectMap.Entry<V>>() {
				final Comparator<? super it.unimi.dsi.fastutil.bytes.Byte2ObjectMap.Entry<V>> comparator = (x, y) -> Byte2ObjectAVLTreeMap.this.actualComparator
	...
	private class EntryIterator extends Byte2ObjectAVLTreeMap<V>.TreeIterator implements ObjectListIterator<it.unimi.dsi.fastutil.bytes.Byte2ObjectMap.Entry<V>> {
		EntryIterator() {
		}
	...
	private final class Submap extends AbstractByte2ObjectSortedMap<V> implements Serializable {
		...
		public V get(byte k) {
			...
			return this.in(k) && (e = Byte2ObjectAVLTreeMap.this.findKey(k)) != null ? e.value : this.defRetValue;
		}
		...
		private class SubmapEntryIterator
			extends Byte2ObjectAVLTreeMap<V>.Submap.SubmapIterator
