...
public class Byte2ObjectOpenHashMap<V> extends AbstractByte2ObjectMap<V> implements Serializable, Cloneable, Hash {
	...
	private void writeObject(ObjectOutputStream s) throws IOException {
		byte[] key = this.key;
		V[] value = this.value;
		Byte2ObjectOpenHashMap<V>.MapIterator i = new Byte2ObjectOpenHashMap.MapIterator();
	...
	private class EntryIterator extends Byte2ObjectOpenHashMap<V>.MapIterator implements ObjectIterator<it.unimi.dsi.fastutil.bytes.Byte2ObjectMap.Entry<V>> {
	...
	private class FastEntryIterator extends Byte2ObjectOpenHashMap<V>.MapIterator implements ObjectIterator<it.unimi.dsi.fastutil.bytes.Byte2ObjectMap.Entry<V>> {
	...
	private final class MapEntrySet extends AbstractObjectSet<it.unimi.dsi.fastutil.bytes.Byte2ObjectMap.Entry<V>> implements FastEntrySet<V> {
		...
		public void forEach(Consumer<? super it.unimi.dsi.fastutil.bytes.Byte2ObjectMap.Entry<V>> consumer) {
			if (Byte2ObjectOpenHashMap.this.containsNullKey) {
				consumer.accept(
					new BasicEntry(Byte2ObjectOpenHashMap.this.key[Byte2ObjectOpenHashMap.this.n], Byte2ObjectOpenHashMap.this.value[Byte2ObjectOpenHashMap.this.n])
				);
			}
			...
			while (pos-- != 0) {
				if (Byte2ObjectOpenHashMap.this.key[pos] != 0) {
					consumer.accept(new BasicEntry(Byte2ObjectOpenHashMap.this.key[pos], Byte2ObjectOpenHashMap.this.value[pos]));
				}
			}
		}
