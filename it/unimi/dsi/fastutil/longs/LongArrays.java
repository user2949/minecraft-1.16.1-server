...
public final class LongArrays {
	...
	public static final Strategy<long[]> HASH_STRATEGY = new LongArrays.ArrayHashStrategy();
	...
	public static void mergeSort(long[] a, int from, int to) {
		mergeSort(a, from, to, (long[])a.clone());
	}
	...
	public static void mergeSort(long[] a, int from, int to, LongComparator comp) {
		mergeSort(a, from, to, comp, (long[])a.clone());
	}
	...
	public static void radixSort(long[] a, int from, int to) {
		if (to - from < 1024) {
			...
			while (stackPos > 0) {
				...
				while (i-- != first) {
					count[(int)(a[i] >>> shift & 255L ^ (long)signMask)]++;
				}
			...
			while (j-- != 0) {
				executorCompletionService.submit(() -> {
					...
					while (true) {
						...
						while (ixxx-- != first) {
							count[(int)(a[ixxx] >>> shift & 255L ^ (long)signMask)]++;
						}
						...
						while (ixx <= ix) {
							...
							if (level < 7 && count[c] > 1) {
								if (count[c] < 1024) {
									...
									queue.add(new LongArrays.Segment(ixx, count[c], level + 1));
								}
							}
	...
	public static void radixSortIndirect(int[] perm, long[] a, int from, int to, boolean stable) {
		if (to - from < 1024) {
			...
			while (stackPos > 0) {
				...
				while (i-- != first) {
					count[(int)(a[perm[i]] >>> shift & 255L ^ (long)signMask)]++;
				}
				...
				if (stable) {
					...
					while (ix-- != first) {
						support[--pos[(int)(a[perm[ix]] >>> shift & 255L ^ (long)signMask)]] = perm[ix];
					}
			...
			while (j-- != 0) {
				executorCompletionService.submit(() -> {
					...
					while (true) {
						...
						while (ixx-- != first) {
							count[(int)(a[perm[ixx]] >>> shift & 255L ^ (long)signMask)]++;
						}
	...
	public static void radixSort(long[] a, long[] b, int from, int to) {
		if (to - from < 1024) {
			...
			while (stackPos > 0) {
				...
				while (i-- != first) {
					count[(int)(k[i] >>> shift & 255L ^ (long)signMask)]++;
				}
				...
				while (ixx <= ix) {
					long t = a[ixx];
					long u = b[ixx];
					c = (int)(k[ixx] >>> shift & 255L ^ (long)signMask);
					if (ixx < ix) {
						int d;
						while ((d = --pos[c]) > ixx) {
				...
				while (j-- != 0) {
					executorCompletionService.submit(() -> {
						...
						while (true) {
							...
							while (ixxxx-- != first) {
								count[(int)(k[ixxxx] >>> shift & 255L ^ (long)signMask)]++;
							}
	...
	public static void radixSortIndirect(int[] perm, long[] a, long[] b, int from, int to, boolean stable) {
		if (to - from < 1024) {
			...
			while (stackPos > 0) {
				...
				while (i-- != first) {
					count[(int)(k[perm[i]] >>> shift & 255L ^ (long)signMask)]++;
				}
				...
				if (stable) {
					...
					while (ix-- != first) {
						support[--pos[(int)(k[perm[ix]] >>> shift & 255L ^ (long)signMask)]] = perm[ix];
					}
	...
	private static final class ArrayHashStrategy implements Strategy<long[]>, Serializable {
				...
				while (true) {
					...
					while (b > c || (comparison = Long.compare(x[b], v)) > 0) {
						...
						if (b > c) {
							...
							comparison = d - c;
							if (s > 1 && comparison > 1) {
								invokeAll(new LongArrays.ForkJoinQuickSort(x, this.from, this.from + s), new LongArrays.ForkJoinQuickSort(x, this.to - comparison, this.to));
							} else if (s > 1) {
								invokeAll(new ForkJoinTask[]{new LongArrays.ForkJoinQuickSort(x, this.from, this.from + s)});
							} else {
								invokeAll(new ForkJoinTask[]{new LongArrays.ForkJoinQuickSort(x, this.to - comparison, this.to)});
							}
				...
				while (true) {
					...
					while (b > c || (comparison = (t = Long.compare(x[b], v)) == 0 ? Long.compare(y[b], w) : t) > 0) {
						...
						if (b > c) {
							...
							comparison = d - c;
							if (s > 1 && comparison > 1) {
								invokeAll(new LongArrays.ForkJoinQuickSort2(x, y, this.from, this.from + s), new LongArrays.ForkJoinQuickSort2(x, y, this.to - comparison, this.to));
							} else if (s > 1) {
								invokeAll(new ForkJoinTask[]{new LongArrays.ForkJoinQuickSort2(x, y, this.from, this.from + s)});
							} else {
								invokeAll(new ForkJoinTask[]{new LongArrays.ForkJoinQuickSort2(x, y, this.to - comparison, this.to)});
							}
					...
					while (b > c || (comparison = this.comp.compare(x[b], v)) > 0) {
						...
						if (b > c) {
							...
							if (s > 1 && comparison > 1) {
								...
								invokeAll(new ForkJoinTask[]{new LongArrays.ForkJoinQuickSortComp(x, this.to - comparison, this.to, this.comp)});
							}
					...
					while (b > c || (comparison = Long.compare(x[this.perm[b]], v)) > 0) {
						...
						if (b > c) {
							...
							if (s > 1 && comparison > 1) {
								...
								invokeAll(new ForkJoinTask[]{new LongArrays.ForkJoinQuickSortIndirect(this.perm, x, this.to - comparison, this.to)});
							}
