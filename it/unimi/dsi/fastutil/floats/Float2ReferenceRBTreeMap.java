...
public class Float2ReferenceRBTreeMap<V> extends AbstractFloat2ReferenceSortedMap<V> implements Serializable, Cloneable {
	...
	protected transient ObjectSortedSet<it.unimi.dsi.fastutil.floats.Float2ReferenceMap.Entry<V>> entries;
				...
				while (true) {
					...
					if (!this.dirPath[i - 1]) {
						...
						if (this.nodePath[i - 1].succ() || y.black()) {
							...
							Float2ReferenceRBTreeMap.Entry<V> xx = this.nodePath[i - 1];
							xx.black(false);
							y.black(true);
							xx.left = y.right;
							y.right = xx;
						...
						if (this.nodePath[i - 1].pred() || y.black()) {
							...
							Float2ReferenceRBTreeMap.Entry<V> xxx = this.nodePath[i - 1];
							xxx.black(false);
							y.black(true);
							xxx.right = y.left;
							y.left = xxx;
			...
			if (p.black()) {
				for (; i > 0; i--) {
					...
					if (!this.dirPath[i - 1]) {
						...
						Float2ReferenceRBTreeMap.Entry<V> wx = this.nodePath[i - 1].left;
						if (!wx.black()) {
							wx.black(true);
							this.nodePath[i - 1].black(false);
						...
						if (!wx.pred() && !wx.left.black() || !wx.succ() && !wx.right.black()) {
							if (wx.pred() || wx.left.black()) {
								Float2ReferenceRBTreeMap.Entry<V> y = wx.right;
								y.black(true);
								wx.black(false);
								wx.right = y.left;
								y.left = wx;
	...
	@Override
	public ObjectSortedSet<it.unimi.dsi.fastutil.floats.Float2ReferenceMap.Entry<V>> float2ReferenceEntrySet() {
		if (this.entries == null) {
			this.entries = new AbstractObjectSortedSet<it.unimi.dsi.fastutil.floats.Float2ReferenceMap.Entry<V>>() {
				final Comparator<? super it.unimi.dsi.fastutil.floats.Float2ReferenceMap.Entry<V>> comparator = (x, y) -> Float2ReferenceRBTreeMap.this.actualComparator
				...
				public ObjectSortedSet<it.unimi.dsi.fastutil.floats.Float2ReferenceMap.Entry<V>> subSet(
					it.unimi.dsi.fastutil.floats.Float2ReferenceMap.Entry<V> from, it.unimi.dsi.fastutil.floats.Float2ReferenceMap.Entry<V> to
				) {
					return Float2ReferenceRBTreeMap.this.subMap(from.getFloatKey(), to.getFloatKey()).float2ReferenceEntrySet();
				}

				public ObjectSortedSet<it.unimi.dsi.fastutil.floats.Float2ReferenceMap.Entry<V>> headSet(it.unimi.dsi.fastutil.floats.Float2ReferenceMap.Entry<V> to) {
					return Float2ReferenceRBTreeMap.this.headMap(to.getFloatKey()).float2ReferenceEntrySet();
				}

				public ObjectSortedSet<it.unimi.dsi.fastutil.floats.Float2ReferenceMap.Entry<V>> tailSet(it.unimi.dsi.fastutil.floats.Float2ReferenceMap.Entry<V> from) {
	...
	private Float2ReferenceRBTreeMap.Entry<V> readTree(ObjectInputStream s, int n, Float2ReferenceRBTreeMap.Entry<V> pred, Float2ReferenceRBTreeMap.Entry<V> succ) throws IOException, ClassNotFoundException {
		if (n == 1) {
	...
	private class KeySet extends AbstractFloat2ReferenceSortedMap<V>.KeySet {
		private KeySet() {
			super(Float2ReferenceRBTreeMap.this);
		}
	...
	private final class Submap extends AbstractFloat2ReferenceSortedMap<V> implements Serializable {
		...
		public V get(float k) {
			Float2ReferenceRBTreeMap.Entry<V> e;
			return this.in(k) && (e = Float2ReferenceRBTreeMap.this.findKey(k)) != null ? e.value : this.defRetValue;
		}
