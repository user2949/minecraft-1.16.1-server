...
public class Float2ObjectArrayMap<V> extends AbstractFloat2ObjectMap<V> implements Serializable, Cloneable {
	...
	private final class EntrySet extends AbstractObjectSet<it.unimi.dsi.fastutil.floats.Float2ObjectMap.Entry<V>> implements FastEntrySet<V> {
		private EntrySet() {
		}

		@Override
		public ObjectIterator<it.unimi.dsi.fastutil.floats.Float2ObjectMap.Entry<V>> iterator() {
			return new ObjectIterator<it.unimi.dsi.fastutil.floats.Float2ObjectMap.Entry<V>>() {
				int curr = -1;
				int next = 0;
				...
				public it.unimi.dsi.fastutil.floats.Float2ObjectMap.Entry<V> next() {
					if (!this.hasNext()) {
						...
						return new BasicEntry<>(Float2ObjectArrayMap.this.key[this.curr = this.next], (V)Float2ObjectArrayMap.this.value[this.next++]);
					}
				}
