...
public class Reference2ShortOpenCustomHashMap<K> extends AbstractReference2ShortMap<K> implements Serializable, Cloneable, Hash {
	...
	public Reference2ShortOpenCustomHashMap(int expected, float f, Strategy<K> strategy) {
	...
	public Reference2ShortOpenCustomHashMap(int expected, Strategy<K> strategy) {
		this(expected, 0.75F, strategy);
	}

	public Reference2ShortOpenCustomHashMap(Strategy<K> strategy) {
		this(16, 0.75F, strategy);
	}

	public Reference2ShortOpenCustomHashMap(Map<? extends K, ? extends Short> m, float f, Strategy<K> strategy) {
		this(m.size(), f, strategy);
		this.putAll(m);
	}

	public Reference2ShortOpenCustomHashMap(Map<? extends K, ? extends Short> m, Strategy<K> strategy) {
		this(m, 0.75F, strategy);
	}

	public Reference2ShortOpenCustomHashMap(Reference2ShortMap<K> m, float f, Strategy<K> strategy) {
		this(m.size(), f, strategy);
		this.putAll(m);
	}

	public Reference2ShortOpenCustomHashMap(Reference2ShortMap<K> m, Strategy<K> strategy) {
		this(m, 0.75F, strategy);
	}

	public Reference2ShortOpenCustomHashMap(K[] k, short[] v, float f, Strategy<K> strategy) {
	...
	public Reference2ShortOpenCustomHashMap(K[] k, short[] v, Strategy<K> strategy) {
	...
	private void writeObject(ObjectOutputStream s) throws IOException {
		K[] key = this.key;
		short[] value = this.value;
		Reference2ShortOpenCustomHashMap<K>.MapIterator i = new Reference2ShortOpenCustomHashMap.MapIterator();
	...
	private class EntryIterator
		extends Reference2ShortOpenCustomHashMap<K>.MapIterator
		implements ObjectIterator<it.unimi.dsi.fastutil.objects.Reference2ShortMap.Entry<K>> {
	...
	final class MapEntry implements it.unimi.dsi.fastutil.objects.Reference2ShortMap.Entry<K>, java.util.Map.Entry<K, Short> {
		...
		public boolean equals(Object o) {
			if (!(o instanceof java.util.Map.Entry)) {
				...
				return Reference2ShortOpenCustomHashMap.this.strategy.equals(Reference2ShortOpenCustomHashMap.this.key[this.index], (K)e.getKey())
					&& Reference2ShortOpenCustomHashMap.this.value[this.index] == (Short)e.getValue();
			}
		}
	...
	private final class MapEntrySet extends AbstractObjectSet<it.unimi.dsi.fastutil.objects.Reference2ShortMap.Entry<K>> implements FastEntrySet<K> {
		private MapEntrySet() {
		}
		...
		public void forEach(Consumer<? super it.unimi.dsi.fastutil.objects.Reference2ShortMap.Entry<K>> consumer) {
			if (Reference2ShortOpenCustomHashMap.this.containsNullKey) {
				consumer.accept(
					new BasicEntry(
						Reference2ShortOpenCustomHashMap.this.key[Reference2ShortOpenCustomHashMap.this.n],
						Reference2ShortOpenCustomHashMap.this.value[Reference2ShortOpenCustomHashMap.this.n]
					)
				);
			}
	...
	private class MapIterator {
		...
		private void shiftKeys(int pos) {
			...
			while (true) {
				...
				K curr;
				for (pos = pos + 1 & Reference2ShortOpenCustomHashMap.this.mask; (curr = key[pos]) != null; pos = pos + 1 & Reference2ShortOpenCustomHashMap.this.mask) {
