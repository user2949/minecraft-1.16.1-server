...
public class Reference2BooleanArrayMap<K> extends AbstractReference2BooleanMap<K> implements Serializable, Cloneable {
	...
	private final class EntrySet extends AbstractObjectSet<it.unimi.dsi.fastutil.objects.Reference2BooleanMap.Entry<K>> implements FastEntrySet<K> {
		private EntrySet() {
		}

		@Override
		public ObjectIterator<it.unimi.dsi.fastutil.objects.Reference2BooleanMap.Entry<K>> iterator() {
			return new ObjectIterator<it.unimi.dsi.fastutil.objects.Reference2BooleanMap.Entry<K>>() {
				int curr = -1;
				int next = 0;
				...
				public it.unimi.dsi.fastutil.objects.Reference2BooleanMap.Entry<K> next() {
					if (!this.hasNext()) {
						throw new NoSuchElementException();
					} else {
						return new BasicEntry<>((K)Reference2BooleanArrayMap.this.key[this.curr = this.next], Reference2BooleanArrayMap.this.value[this.next++]);
					}
				}
