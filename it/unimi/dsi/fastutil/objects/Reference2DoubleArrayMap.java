...
public class Reference2DoubleArrayMap<K> extends AbstractReference2DoubleMap<K> implements Serializable, Cloneable {
	...
	private final class EntrySet extends AbstractObjectSet<it.unimi.dsi.fastutil.objects.Reference2DoubleMap.Entry<K>> implements FastEntrySet<K> {
		private EntrySet() {
		}

		@Override
		public ObjectIterator<it.unimi.dsi.fastutil.objects.Reference2DoubleMap.Entry<K>> iterator() {
			return new ObjectIterator<it.unimi.dsi.fastutil.objects.Reference2DoubleMap.Entry<K>>() {
				int curr = -1;
				int next = 0;
				...
				public it.unimi.dsi.fastutil.objects.Reference2DoubleMap.Entry<K> next() {
					if (!this.hasNext()) {
						throw new NoSuchElementException();
					} else {
						return new BasicEntry<>((K)Reference2DoubleArrayMap.this.key[this.curr = this.next], Reference2DoubleArrayMap.this.value[this.next++]);
					}
				}
