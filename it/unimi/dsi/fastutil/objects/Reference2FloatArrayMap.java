...
public class Reference2FloatArrayMap<K> extends AbstractReference2FloatMap<K> implements Serializable, Cloneable {
	...
	private final class EntrySet extends AbstractObjectSet<it.unimi.dsi.fastutil.objects.Reference2FloatMap.Entry<K>> implements FastEntrySet<K> {
		private EntrySet() {
		}

		@Override
		public ObjectIterator<it.unimi.dsi.fastutil.objects.Reference2FloatMap.Entry<K>> iterator() {
			return new ObjectIterator<it.unimi.dsi.fastutil.objects.Reference2FloatMap.Entry<K>>() {
				int curr = -1;
				int next = 0;
				...
				public it.unimi.dsi.fastutil.objects.Reference2FloatMap.Entry<K> next() {
					if (!this.hasNext()) {
						throw new NoSuchElementException();
					} else {
						return new BasicEntry<>((K)Reference2FloatArrayMap.this.key[this.curr = this.next], Reference2FloatArrayMap.this.value[this.next++]);
					}
				}
