...
public class ObjectBigArrayBigList<K> extends AbstractObjectBigList<K> implements RandomAccess, Cloneable, Serializable {
	...
	public long indexOf(Object k) {
		for (long i = 0L; i < this.size; i++) {
			if (Objects.equals(k, ObjectBigArrays.get(this.a, i))) {
				return i;
			}
		}
	...
	public ObjectBigArrayBigList<K> clone() {
		ObjectBigArrayBigList<K> c = new ObjectBigArrayBigList<>(this.size);
	...
	public int compareTo(ObjectBigArrayBigList<? extends K> l) {
		...
		for (i = 0; (long)i < s1 && (long)i < s2; i++) {
			K e1 = ObjectBigArrays.get(a1, (long)i);
			K e2 = ObjectBigArrays.get(a2, (long)i);
			int r;
