...
public class Reference2ReferenceArrayMap<K, V> extends AbstractReference2ReferenceMap<K, V> implements Serializable, Cloneable {
	...
	private final class EntrySet extends AbstractObjectSet<it.unimi.dsi.fastutil.objects.Reference2ReferenceMap.Entry<K, V>> implements FastEntrySet<K, V> {
		private EntrySet() {
		}

		@Override
		public ObjectIterator<it.unimi.dsi.fastutil.objects.Reference2ReferenceMap.Entry<K, V>> iterator() {
			return new ObjectIterator<it.unimi.dsi.fastutil.objects.Reference2ReferenceMap.Entry<K, V>>() {
				int curr = -1;
				int next = 0;
				...
				public it.unimi.dsi.fastutil.objects.Reference2ReferenceMap.Entry<K, V> next() {
					if (!this.hasNext()) {
						throw new NoSuchElementException();
					} else {
						return new BasicEntry<>((K)Reference2ReferenceArrayMap.this.key[this.curr = this.next], (V)Reference2ReferenceArrayMap.this.value[this.next++]);
					}
				}
