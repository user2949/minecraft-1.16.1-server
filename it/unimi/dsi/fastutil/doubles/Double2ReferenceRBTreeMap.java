...
public class Double2ReferenceRBTreeMap<V> extends AbstractDouble2ReferenceSortedMap<V> implements Serializable, Cloneable {
				...
				while (true) {
					...
					if (!this.dirPath[i - 1]) {
						...
						if (this.nodePath[i - 1].succ() || y.black()) {
							...
							Double2ReferenceRBTreeMap.Entry<V> xx = this.nodePath[i - 1];
							xx.black(false);
							y.black(true);
							xx.left = y.right;
							y.right = xx;
						...
						if (this.nodePath[i - 1].pred() || y.black()) {
							...
							Double2ReferenceRBTreeMap.Entry<V> xxx = this.nodePath[i - 1];
							xxx.black(false);
							y.black(true);
							xxx.right = y.left;
							y.left = xxx;
			...
			if (p.black()) {
				for (; i > 0; i--) {
					...
					if (!this.dirPath[i - 1]) {
						...
						Double2ReferenceRBTreeMap.Entry<V> wx = this.nodePath[i - 1].left;
						if (!wx.black()) {
							wx.black(true);
							this.nodePath[i - 1].black(false);
						...
						if (!wx.pred() && !wx.left.black() || !wx.succ() && !wx.right.black()) {
							if (wx.pred() || wx.left.black()) {
								Double2ReferenceRBTreeMap.Entry<V> y = wx.right;
								y.black(true);
								wx.black(false);
								wx.right = y.left;
								y.left = wx;
	...
	public ObjectSortedSet<it.unimi.dsi.fastutil.doubles.Double2ReferenceMap.Entry<V>> double2ReferenceEntrySet() {
		if (this.entries == null) {
			this.entries = new AbstractObjectSortedSet<it.unimi.dsi.fastutil.doubles.Double2ReferenceMap.Entry<V>>() {
				final Comparator<? super it.unimi.dsi.fastutil.doubles.Double2ReferenceMap.Entry<V>> comparator = (x, y) -> Double2ReferenceRBTreeMap.this.actualComparator
	...
	private class KeySet extends AbstractDouble2ReferenceSortedMap<V>.KeySet {
		private KeySet() {
			super(Double2ReferenceRBTreeMap.this);
		}
	...
	private final class Submap extends AbstractDouble2ReferenceSortedMap<V> implements Serializable {
		...
		public V get(double k) {
			Double2ReferenceRBTreeMap.Entry<V> e;
			return this.in(k) && (e = Double2ReferenceRBTreeMap.this.findKey(k)) != null ? e.value : this.defRetValue;
		}
		...
		public Double2ReferenceSortedMap<V> headMap(double to) {
			if (this.top) {
				...
				return Double2ReferenceRBTreeMap.this.compare(to, this.to) < 0 ? Double2ReferenceRBTreeMap.this.new Submap(this.from, this.bottom, to, false) : this;
			}
		}
		...
		public Double2ReferenceSortedMap<V> tailMap(double from) {
			if (this.bottom) {
				...
				return Double2ReferenceRBTreeMap.this.compare(from, this.from) > 0 ? Double2ReferenceRBTreeMap.this.new Submap(from, false, this.to, this.top) : this;
			}
		}
		...
		private class SubmapEntryIterator
			extends Double2ReferenceRBTreeMap<V>.Submap.SubmapIterator
			implements ObjectListIterator<it.unimi.dsi.fastutil.doubles.Double2ReferenceMap.Entry<V>> {
			SubmapEntryIterator() {
			}
