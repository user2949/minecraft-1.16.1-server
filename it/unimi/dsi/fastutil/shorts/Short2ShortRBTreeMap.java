...
public class Short2ShortRBTreeMap extends AbstractShort2ShortSortedMap implements Serializable, Cloneable {
				...
				while (true) {
					...
					if (!this.dirPath[i - 1]) {
						...
						if (this.nodePath[i - 1].succ() || y.black()) {
							...
							Short2ShortRBTreeMap.Entry xx = this.nodePath[i - 1];
							xx.black(false);
							y.black(true);
							xx.left = y.right;
							y.right = xx;
						...
						if (this.nodePath[i - 1].pred() || y.black()) {
							...
							Short2ShortRBTreeMap.Entry xxx = this.nodePath[i - 1];
							xxx.black(false);
							y.black(true);
							xxx.right = y.left;
							y.left = xxx;
			...
			if (p.black()) {
				for (; i > 0; i--) {
					...
					if (!this.dirPath[i - 1]) {
						...
						Short2ShortRBTreeMap.Entry wx = this.nodePath[i - 1].left;
						if (!wx.black()) {
							wx.black(true);
							this.nodePath[i - 1].black(false);
						...
						if (!wx.pred() && !wx.left.black() || !wx.succ() && !wx.right.black()) {
							if (wx.pred() || wx.left.black()) {
								Short2ShortRBTreeMap.Entry y = wx.right;
								y.black(true);
								wx.black(false);
								wx.right = y.left;
								y.left = wx;
	...
	public ObjectSortedSet<it.unimi.dsi.fastutil.shorts.Short2ShortMap.Entry> short2ShortEntrySet() {
		if (this.entries == null) {
			this.entries = new AbstractObjectSortedSet<it.unimi.dsi.fastutil.shorts.Short2ShortMap.Entry>() {
				final Comparator<? super it.unimi.dsi.fastutil.shorts.Short2ShortMap.Entry> comparator = (x, y) -> Short2ShortRBTreeMap.this.actualComparator
	...
	private class EntryIterator extends Short2ShortRBTreeMap.TreeIterator implements ObjectListIterator<it.unimi.dsi.fastutil.shorts.Short2ShortMap.Entry> {
		EntryIterator() {
		}
	...
	private class KeySet extends it.unimi.dsi.fastutil.shorts.AbstractShort2ShortSortedMap.KeySet {
		private KeySet() {
			super(Short2ShortRBTreeMap.this);
		}
	...
	private final class Submap extends AbstractShort2ShortSortedMap implements Serializable {
		...
		public short get(short k) {
			Short2ShortRBTreeMap.Entry e;
			return this.in(k) && (e = Short2ShortRBTreeMap.this.findKey(k)) != null ? e.value : this.defRetValue;
		}
		...
		private class SubmapEntryIterator
			extends Short2ShortRBTreeMap.Submap.SubmapIterator
