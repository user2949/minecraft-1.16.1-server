...
public class Short2ByteRBTreeMap extends AbstractShort2ByteSortedMap implements Serializable, Cloneable {
				...
				while (true) {
					...
					if (!this.dirPath[i - 1]) {
						...
						if (this.nodePath[i - 1].succ() || y.black()) {
							...
							Short2ByteRBTreeMap.Entry xx = this.nodePath[i - 1];
							xx.black(false);
							y.black(true);
							xx.left = y.right;
							y.right = xx;
						...
						if (this.nodePath[i - 1].pred() || y.black()) {
							...
							Short2ByteRBTreeMap.Entry xxx = this.nodePath[i - 1];
							xxx.black(false);
							y.black(true);
							xxx.right = y.left;
							y.left = xxx;
			...
			if (p.black()) {
				for (; i > 0; i--) {
					...
					if (!this.dirPath[i - 1]) {
						...
						Short2ByteRBTreeMap.Entry wx = this.nodePath[i - 1].left;
						if (!wx.black()) {
							wx.black(true);
							this.nodePath[i - 1].black(false);
						...
						if (!wx.pred() && !wx.left.black() || !wx.succ() && !wx.right.black()) {
							if (wx.pred() || wx.left.black()) {
								Short2ByteRBTreeMap.Entry y = wx.right;
								y.black(true);
								wx.black(false);
								wx.right = y.left;
								y.left = wx;
	...
	public ObjectSortedSet<it.unimi.dsi.fastutil.shorts.Short2ByteMap.Entry> short2ByteEntrySet() {
		if (this.entries == null) {
			this.entries = new AbstractObjectSortedSet<it.unimi.dsi.fastutil.shorts.Short2ByteMap.Entry>() {
				final Comparator<? super it.unimi.dsi.fastutil.shorts.Short2ByteMap.Entry> comparator = (x, y) -> Short2ByteRBTreeMap.this.actualComparator
	...
	private class EntryIterator extends Short2ByteRBTreeMap.TreeIterator implements ObjectListIterator<it.unimi.dsi.fastutil.shorts.Short2ByteMap.Entry> {
		EntryIterator() {
		}
	...
	private final class Submap extends AbstractShort2ByteSortedMap implements Serializable {
		...
		public byte get(short k) {
			Short2ByteRBTreeMap.Entry e;
			return this.in(k) && (e = Short2ByteRBTreeMap.this.findKey(k)) != null ? e.value : this.defRetValue;
		}
