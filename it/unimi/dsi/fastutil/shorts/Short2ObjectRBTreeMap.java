...
public class Short2ObjectRBTreeMap<V> extends AbstractShort2ObjectSortedMap<V> implements Serializable, Cloneable {
				...
				while (true) {
					...
					if (!this.dirPath[i - 1]) {
						...
						if (this.nodePath[i - 1].succ() || y.black()) {
							...
							Short2ObjectRBTreeMap.Entry<V> xx = this.nodePath[i - 1];
							xx.black(false);
							y.black(true);
							xx.left = y.right;
							y.right = xx;
						...
						if (this.nodePath[i - 1].pred() || y.black()) {
							...
							Short2ObjectRBTreeMap.Entry<V> xxx = this.nodePath[i - 1];
							xxx.black(false);
							y.black(true);
							xxx.right = y.left;
							y.left = xxx;
			...
			if (p.black()) {
				for (; i > 0; i--) {
					...
					if (!this.dirPath[i - 1]) {
						...
						Short2ObjectRBTreeMap.Entry<V> wx = this.nodePath[i - 1].left;
						if (!wx.black()) {
							wx.black(true);
							this.nodePath[i - 1].black(false);
						...
						if (!wx.pred() && !wx.left.black() || !wx.succ() && !wx.right.black()) {
							if (wx.pred() || wx.left.black()) {
								Short2ObjectRBTreeMap.Entry<V> y = wx.right;
								y.black(true);
								wx.black(false);
								wx.right = y.left;
								y.left = wx;
	...
	public ObjectSortedSet<it.unimi.dsi.fastutil.shorts.Short2ObjectMap.Entry<V>> short2ObjectEntrySet() {
		if (this.entries == null) {
			this.entries = new AbstractObjectSortedSet<it.unimi.dsi.fastutil.shorts.Short2ObjectMap.Entry<V>>() {
				final Comparator<? super it.unimi.dsi.fastutil.shorts.Short2ObjectMap.Entry<V>> comparator = (x, y) -> Short2ObjectRBTreeMap.this.actualComparator
	...
	private class EntryIterator extends Short2ObjectRBTreeMap<V>.TreeIterator implements ObjectListIterator<it.unimi.dsi.fastutil.shorts.Short2ObjectMap.Entry<V>> {
		EntryIterator() {
		}
	...
	private class KeySet extends AbstractShort2ObjectSortedMap<V>.KeySet {
		private KeySet() {
			super(Short2ObjectRBTreeMap.this);
		}
	...
	private final class Submap extends AbstractShort2ObjectSortedMap<V> implements Serializable {
		...
		public V get(short k) {
			Short2ObjectRBTreeMap.Entry<V> e;
			return this.in(k) && (e = Short2ObjectRBTreeMap.this.findKey(k)) != null ? e.value : this.defRetValue;
		}
