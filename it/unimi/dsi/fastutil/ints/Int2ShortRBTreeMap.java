...
public class Int2ShortRBTreeMap extends AbstractInt2ShortSortedMap implements Serializable, Cloneable {
				...
				while (true) {
					...
					if (!this.dirPath[i - 1]) {
						...
						if (this.nodePath[i - 1].succ() || y.black()) {
							...
							Int2ShortRBTreeMap.Entry xx = this.nodePath[i - 1];
							xx.black(false);
							y.black(true);
							xx.left = y.right;
							y.right = xx;
						...
						if (this.nodePath[i - 1].pred() || y.black()) {
							...
							Int2ShortRBTreeMap.Entry xxx = this.nodePath[i - 1];
							xxx.black(false);
							y.black(true);
							xxx.right = y.left;
							y.left = xxx;
			...
			if (p.black()) {
				for (; i > 0; i--) {
					...
					if (!this.dirPath[i - 1]) {
						...
						Int2ShortRBTreeMap.Entry wx = this.nodePath[i - 1].left;
						if (!wx.black()) {
							wx.black(true);
							this.nodePath[i - 1].black(false);
						...
						if (!wx.pred() && !wx.left.black() || !wx.succ() && !wx.right.black()) {
							if (wx.pred() || wx.left.black()) {
								Int2ShortRBTreeMap.Entry y = wx.right;
								y.black(true);
								wx.black(false);
								wx.right = y.left;
								y.left = wx;
	...
	public ObjectSortedSet<it.unimi.dsi.fastutil.ints.Int2ShortMap.Entry> int2ShortEntrySet() {
		if (this.entries == null) {
			this.entries = new AbstractObjectSortedSet<it.unimi.dsi.fastutil.ints.Int2ShortMap.Entry>() {
				final Comparator<? super it.unimi.dsi.fastutil.ints.Int2ShortMap.Entry> comparator = (x, y) -> Int2ShortRBTreeMap.this.actualComparator
	...
	private class EntryIterator extends Int2ShortRBTreeMap.TreeIterator implements ObjectListIterator<it.unimi.dsi.fastutil.ints.Int2ShortMap.Entry> {
		EntryIterator() {
		}
	...
	private final class Submap extends AbstractInt2ShortSortedMap implements Serializable {
		...
		public short get(int k) {
			Int2ShortRBTreeMap.Entry e;
			return this.in(k) && (e = Int2ShortRBTreeMap.this.findKey(k)) != null ? e.value : this.defRetValue;
		}
		...
		private class SubmapEntryIterator
			extends Int2ShortRBTreeMap.Submap.SubmapIterator
