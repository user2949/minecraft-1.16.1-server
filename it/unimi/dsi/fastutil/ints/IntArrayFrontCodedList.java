...
public class IntArrayFrontCodedList extends AbstractObjectList<int[]> implements Serializable, Cloneable, RandomAccess {
		...
		if (ratio < 1) {
			...
			for (int b = 0; arrays.hasNext(); n++) {
				...
				if (n % ratio == 0) {
					...
					p[n / ratio] = curSize;
					array = IntBigArrays.grow(array, curSize + (long)count(length) + (long)length, curSize);
					curSize += (long)writeInt(array, length, curSize);
					IntBigArrays.copyToBig(a[b], 0, array, curSize, (long)length);
					curSize += (long)length;
				} else {
					...
					length -= common;
					array = IntBigArrays.grow(array, curSize + (long)count(length) + (long)count(common) + (long)length, curSize);
					curSize += (long)writeInt(array, length, curSize);
					curSize += (long)writeInt(array, common, curSize);
					IntBigArrays.copyToBig(a[b], common, array, curSize, (long)length);
					curSize += (long)length;
				}
	...
	private int extract(int index, int[] a, int offset, int length) {
		...
		if (delta == 0) {
			...
			if (currLen < length) {
				IntBigArrays.copyFromBig(this.array, pos + (long)count(arrayLength) + (long)count(common), a, currLen + offset, Math.min(arrayLength, length - currLen));
			}
	...
	@Override
	public ObjectListIterator<int[]> listIterator(int start) {
