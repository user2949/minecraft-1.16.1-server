...
public class Int2LongRBTreeMap extends AbstractInt2LongSortedMap implements Serializable, Cloneable {
				...
				while (true) {
					...
					if (!this.dirPath[i - 1]) {
						...
						if (this.nodePath[i - 1].succ() || y.black()) {
							...
							Int2LongRBTreeMap.Entry xx = this.nodePath[i - 1];
							xx.black(false);
							y.black(true);
							xx.left = y.right;
							y.right = xx;
						...
						if (this.nodePath[i - 1].pred() || y.black()) {
							...
							Int2LongRBTreeMap.Entry xxx = this.nodePath[i - 1];
							xxx.black(false);
							y.black(true);
							xxx.right = y.left;
							y.left = xxx;
			...
			if (p.black()) {
				for (; i > 0; i--) {
					...
					if (!this.dirPath[i - 1]) {
						...
						Int2LongRBTreeMap.Entry wx = this.nodePath[i - 1].left;
						if (!wx.black()) {
							wx.black(true);
							this.nodePath[i - 1].black(false);
						...
						if (!wx.pred() && !wx.left.black() || !wx.succ() && !wx.right.black()) {
							if (wx.pred() || wx.left.black()) {
								Int2LongRBTreeMap.Entry y = wx.right;
								y.black(true);
								wx.black(false);
								wx.right = y.left;
								y.left = wx;
	...
	public ObjectSortedSet<it.unimi.dsi.fastutil.ints.Int2LongMap.Entry> int2LongEntrySet() {
		if (this.entries == null) {
			this.entries = new AbstractObjectSortedSet<it.unimi.dsi.fastutil.ints.Int2LongMap.Entry>() {
				final Comparator<? super it.unimi.dsi.fastutil.ints.Int2LongMap.Entry> comparator = (x, y) -> Int2LongRBTreeMap.this.actualComparator
	...
	private class EntryIterator extends Int2LongRBTreeMap.TreeIterator implements ObjectListIterator<it.unimi.dsi.fastutil.ints.Int2LongMap.Entry> {
		EntryIterator() {
		}
	...
	private final class Submap extends AbstractInt2LongSortedMap implements Serializable {
		...
		public long get(int k) {
			Int2LongRBTreeMap.Entry e;
			return this.in(k) && (e = Int2LongRBTreeMap.this.findKey(k)) != null ? e.value : this.defRetValue;
		}
		...
		private class SubmapEntryIterator extends Int2LongRBTreeMap.Submap.SubmapIterator implements ObjectListIterator<it.unimi.dsi.fastutil.ints.Int2LongMap.Entry> {
			SubmapEntryIterator() {
			}
