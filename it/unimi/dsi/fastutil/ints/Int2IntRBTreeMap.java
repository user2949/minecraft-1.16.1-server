...
public class Int2IntRBTreeMap extends AbstractInt2IntSortedMap implements Serializable, Cloneable {
				...
				while (true) {
					...
					if (!this.dirPath[i - 1]) {
						...
						if (this.nodePath[i - 1].succ() || y.black()) {
							...
							Int2IntRBTreeMap.Entry xx = this.nodePath[i - 1];
							xx.black(false);
							y.black(true);
							xx.left = y.right;
							y.right = xx;
						...
						if (this.nodePath[i - 1].pred() || y.black()) {
							...
							Int2IntRBTreeMap.Entry xxx = this.nodePath[i - 1];
							xxx.black(false);
							y.black(true);
							xxx.right = y.left;
							y.left = xxx;
			...
			if (p.black()) {
				for (; i > 0; i--) {
					...
					if (!this.dirPath[i - 1]) {
						...
						Int2IntRBTreeMap.Entry wx = this.nodePath[i - 1].left;
						if (!wx.black()) {
							wx.black(true);
							this.nodePath[i - 1].black(false);
						...
						if (!wx.pred() && !wx.left.black() || !wx.succ() && !wx.right.black()) {
							if (wx.pred() || wx.left.black()) {
								Int2IntRBTreeMap.Entry y = wx.right;
								y.black(true);
								wx.black(false);
								wx.right = y.left;
								y.left = wx;
	...
	public ObjectSortedSet<it.unimi.dsi.fastutil.ints.Int2IntMap.Entry> int2IntEntrySet() {
		if (this.entries == null) {
			this.entries = new AbstractObjectSortedSet<it.unimi.dsi.fastutil.ints.Int2IntMap.Entry>() {
				final Comparator<? super it.unimi.dsi.fastutil.ints.Int2IntMap.Entry> comparator = (x, y) -> Int2IntRBTreeMap.this.actualComparator
	...
	private class EntryIterator extends Int2IntRBTreeMap.TreeIterator implements ObjectListIterator<it.unimi.dsi.fastutil.ints.Int2IntMap.Entry> {
		EntryIterator() {
		}
	...
	private final class Submap extends AbstractInt2IntSortedMap implements Serializable {
		...
		public int get(int k) {
			Int2IntRBTreeMap.Entry e;
			return this.in(k) && (e = Int2IntRBTreeMap.this.findKey(k)) != null ? e.value : this.defRetValue;
		}
		...
		private class SubmapEntryIterator extends Int2IntRBTreeMap.Submap.SubmapIterator implements ObjectListIterator<it.unimi.dsi.fastutil.ints.Int2IntMap.Entry> {
			SubmapEntryIterator() {
			}
