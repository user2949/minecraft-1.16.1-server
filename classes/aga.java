...
public class aga extends DataFix {
	private static final Map<String, Integer> a = DataFixUtils.make(Maps.<String, Integer>newHashMap(), hashMap -> {
	...
	public aga(Schema schema, boolean boolean2) {
		super(schema, boolean2);
	}

	public static int a(String string) {
		Integer integer2 = (Integer)a.get(string);
	...
	public TypeRewriteRule makeRule() {
		...
		Function<Typed<?>, Typed<?>> function4 = typed -> this.a(typed, "DisplayTile", "DisplayData", "DisplayState");
		Function<Typed<?>, Typed<?>> function5 = typed -> this.a(typed, "inTile", "inData", "inBlockState");
		Type<Pair<Either<Pair<String, Either<Integer, String>>, Unit>, Dynamic<?>>> type6 = DSL.and(
			DSL.optional(DSL.field("inTile", DSL.named(ajb.q.typeName(), DSL.or(DSL.intType(), aka.a())))), DSL.remainderType()
		);
		Function<Typed<?>, Typed<?>> function7 = typed -> typed.update(type6.finder(), DSL.remainderType(), Pair::getSecond);
		return this.fixTypeEverywhereTyped("EntityBlockStateFix", schema2.getType(ajb.p), schema3.getType(ajb.p), typed -> {
			typed = this.a(typed, "minecraft:falling_block", this::a);
			typed = this.a(typed, "minecraft:enderman", typedx -> this.a(typedx, "carried", "carriedData", "carriedBlockState"));
			typed = this.a(typed, "minecraft:arrow", function5);
			typed = this.a(typed, "minecraft:spectral_arrow", function5);
			...
			typed = this.a(typed, "minecraft:commandblock_minecart", function4);
			typed = this.a(typed, "minecraft:minecart", function4);
			typed = this.a(typed, "minecraft:chest_minecart", function4);
			typed = this.a(typed, "minecraft:furnace_minecart", function4);
			typed = this.a(typed, "minecraft:tnt_minecart", function4);
			typed = this.a(typed, "minecraft:hopper_minecart", function4);
			return this.a(typed, "minecraft:spawner_minecart", function4);
		});
	}

	private Typed<?> a(Typed<?> typed) {
		Type<Either<Pair<String, Either<Integer, String>>, Unit>> type3 = DSL.optional(
			DSL.field("Block", DSL.named(ajb.q.typeName(), DSL.or(DSL.intType(), aka.a())))
		);
		Type<Either<Pair<String, Dynamic<?>>, Unit>> type4 = DSL.optional(DSL.field("BlockState", DSL.named(ajb.m.typeName(), DSL.remainderType())));
		Dynamic<?> dynamic5 = typed.get(DSL.remainderFinder());
		return typed.update(type3.finder(), type4, either -> {
			int integer3 = either.<Integer>map(pair -> ((Either)pair.getSecond()).map(integer -> integer, aga::a), unit -> {
				Optional<Number> optional3 = dynamic5.get("TileID").asNumber().result();
				return (Integer)optional3.map(Number::intValue).orElseGet(() -> dynamic5.get("Tile").asByte((byte)0) & 0xFF);
			});
			int integer4 = dynamic5.get("Data").asInt(0) & 15;
			return Either.left(Pair.of(ajb.m.typeName(), afn.b(integer3 << 4 | integer4)));
		}).set(DSL.remainderFinder(), dynamic5.remove("Data").remove("TileID").remove("Tile"));
	}

	private Typed<?> a(Typed<?> typed, String string2, String string3, String string4) {
		Type<Pair<String, Either<Integer, String>>> type6 = DSL.field(string2, DSL.named(ajb.q.typeName(), DSL.or(DSL.intType(), aka.a())));
		Type<Pair<String, Dynamic<?>>> type7 = DSL.field(string4, DSL.named(ajb.m.typeName(), DSL.remainderType()));
		Dynamic<?> dynamic8 = typed.getOrCreate(DSL.remainderFinder());
		return typed.update(type6.finder(), type7, pair -> {
			int integer4 = ((Either)pair.getSecond()).<Integer>map(integer -> integer, aga::a);
			int integer5 = dynamic8.get(string3).asInt(0) & 15;
			return Pair.of(ajb.m.typeName(), afn.b(integer4 << 4 | integer5));
		}).set(DSL.remainderFinder(), dynamic8.remove(string3));
	}

	private Typed<?> a(Typed<?> typed, String string, Function<Typed<?>, Typed<?>> function) {
