...
public class afv extends DataFix {
	private static final ImmutableMap<String, Pair<String, ImmutableMap<String, String>>> a = ImmutableMap.<String, Pair<String, ImmutableMap<String, String>>>builder(
	...
	public afv(Schema schema, boolean boolean2) {
	...
	public TypeRewriteRule makeRule() {
		...
		OpticFinder<?> opticFinder3 = type2.findField("Children");
		return this.fixTypeEverywhereTyped(
			"ChunkStructuresTemplateRenameFix",
			type2,
			typed -> typed.updateTyped(opticFinder3, typed2 -> typed2.update(DSL.remainderFinder(), dynamic -> this.a(typed.get(DSL.remainderFinder()), dynamic)))
		);
	}

	private Dynamic<?> a(Dynamic<?> dynamic1, Dynamic<?> dynamic2) {
		String string4 = dynamic1.get("id").asString("");
		if (a.containsKey(string4)) {
			Pair<String, ImmutableMap<String, String>> pair5 = a.get(string4);
			if (pair5.getFirst().equals(dynamic2.get("id").asString(""))) {
				String string6 = dynamic2.get("Template").asString("");
				dynamic2 = dynamic2.set("Template", dynamic2.createString(pair5.getSecond().getOrDefault(string6, string6)));
			}
		}
