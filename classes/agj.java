...
public class agj extends DataFix {
	...
	public agj(Schema schema, boolean boolean2) {
	...
	public TypeRewriteRule makeRule() {
		...
		Type<?> type4 = this.getInputSchema().getType(ajb.l);
		Type<?> type5 = this.getOutputSchema().getType(ajb.l);
		return TypeRewriteRule.seq(
			this.convertUnchecked("item stack entity name hook converter", type4, type5),
			this.fixTypeEverywhere(
				"EntityIdFix", taggedChoiceType2, taggedChoiceType3, dynamicOps -> pair -> pair.mapFirst(string -> (String)a.getOrDefault(string, string))
			)
		);
	}
}
