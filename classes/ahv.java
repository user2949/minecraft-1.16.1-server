...
public class ahv extends DataFix {
	...
	private static final Set<String> b = (Set<String>)a.keySet().stream().map(string -> string.substring(0, string.indexOf(46))).collect(Collectors.toSet());
	...
	public ahv(Schema schema, boolean boolean2) {
	...
	public TypeRewriteRule makeRule() {
		...
		return this.fixTypeEverywhereTyped("ItemInstanceTheFlatteningFix", type2, typed -> {
			...
			if (!optional4.isPresent()) {
				...
				int integer7 = dynamic6.get("Damage").asInt(0);
				String string8 = a((String)((Pair)optional4.get()).getSecond(), integer7);
				if (string8 != null) {
					typed5 = typed.set(opticFinder3, Pair.of(ajb.r.typeName(), string8));
				}

				if (c.contains(((Pair)optional4.get()).getSecond())) {
					Typed<?> typed9 = typed.getOrCreateTyped(opticFinder4);
					Dynamic<?> dynamic10 = typed9.get(DSL.remainderFinder());
					dynamic10 = dynamic10.set("Damage", dynamic10.createInt(integer7));
					typed5 = typed5.set(opticFinder4, typed9.set(DSL.remainderFinder(), dynamic10));
				}

				return typed5.set(DSL.remainderFinder(), dynamic6.remove("Damage"));
			}
		});
	}

	@Nullable
	public static String a(@Nullable String string, int integer) {
		if (b.contains(string)) {
			String string3 = (String)a.get(string + '.' + integer);
			return string3 == null ? (String)a.get(string + ".0") : string3;
		} else {
			return null;
		}
	}
