...
public class air extends DataFix {
	...
	public air(Schema schema, boolean boolean2) {
					...
					dynamic -> (Dynamic)dynamic.getMapValues()
							.map(map -> dynamic.createMap((Map<? extends Dynamic<?>, ? extends Dynamic<?>>)map.entrySet().stream().map(entry -> {
									if (((Dynamic)entry.getKey()).asString("").startsWith("key_")) {
										int integer2 = Integer.parseInt(((Dynamic)entry.getValue()).asString(""));
										if (integer2 < 0) {
											int integer3 = integer2 + 100;
											String string4;
											if (integer3 == 0) {
												...
												string4 = "key.mouse." + (integer3 + 1);
											}
			
											return Pair.of(entry.getKey(), ((Dynamic)entry.getValue()).createString(string4));
										} else {
											String string3 = a.getOrDefault(integer2, "key.unknown");
											return Pair.of(entry.getKey(), ((Dynamic)entry.getValue()).createString(string3));
										}
									} else {
