...
public class LoggerNameLevelRewritePolicy implements RewritePolicy {
	...
	public static LoggerNameLevelRewritePolicy createPolicy(
		@PluginAttribute("logger") String loggerNamePrefix, @PluginElement("KeyValuePair") KeyValuePair[] levelPairs
	) {
	...
	public LogEvent rewrite(LogEvent event) {
		if (!event.getLoggerName().startsWith(this.loggerName)) {
			...
			if (newLevel != null && newLevel != sourceLevel) {
				LogEvent result = new Builder(event).setLevel(newLevel).build();
				return result;
			} else {
				return event;
			}
