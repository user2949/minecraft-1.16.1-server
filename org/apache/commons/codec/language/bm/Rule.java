...
public class Rule {
	...
	private static Rule.Phoneme parsePhoneme(String ph) {
		...
		if (open >= 0) {
			if (!ph.endsWith("]")) {
				...
				return new Rule.Phoneme(before, LanguageSet.from(langs));
			}
		} else {
	...
	public static final class Phoneme implements Rule.PhonemeExpr {
		...
		public Phoneme(Rule.Phoneme phonemeLeft, Rule.Phoneme phonemeRight, LanguageSet languages) {
		...
		public Rule.Phoneme mergeWithLanguage(LanguageSet lang) {
