...
public class XmlStreamReader extends Reader {
	...
	private static final ByteOrderMark[] XML_GUESS_BYTES = new ByteOrderMark[]{
		new ByteOrderMark("UTF-8", 60, 63, 120, 109),
		new ByteOrderMark("UTF-16BE", 0, 60, 0, 63),
		new ByteOrderMark("UTF-16LE", 60, 0, 63, 0),
	...
	private String doRawStream(BOMInputStream bom, BOMInputStream pis, boolean lenient) throws IOException {
		...
		try {
			...
			if (lenient) {
				return this.doLenientDetection(null, var8);
			} else {
				throw var8;
			}
		}
		...
		if (bomEnc != null) {
			if (bomEnc.equals("UTF-8")) {
				if (xmlGuessEnc != null && !xmlGuessEnc.equals("UTF-8")) {
					String msg = MessageFormat.format("Invalid encoding, BOM [{0}] XML guess [{1}] XML prolog [{2}] encoding mismatch", bomEnc, xmlGuessEnc, xmlEnc);
		...
		if (lenient && xmlEnc != null) {
			...
			if (!appXml && !textXml) {
				String msg = MessageFormat.format(
					"Invalid encoding, CT-MIME [{0}] CT-Enc [{1}] BOM [{2}] XML guess [{3}] XML prolog [{4}], Invalid MIME", cTMime, cTEnc, bomEnc, xmlGuessEnc, xmlEnc
				);
				...
				if (cTEnc.equals("UTF-16")) {
					if (bomEnc != null && bomEnc.startsWith("UTF-16")) {
						...
						String msg = MessageFormat.format(
							"Invalid encoding, CT-MIME [{0}] CT-Enc [{1}] BOM [{2}] XML guess [{3}] XML prolog [{4}], encoding mismatch", cTMime, cTEnc, bomEnc, xmlGuessEnc, xmlEnc
						);
						throw new XmlStreamReaderException(msg, cTMime, cTEnc, bomEnc, xmlGuessEnc, xmlEnc);
					}
					...
					if (!cTEnc.equals("UTF-32")) {
						return cTEnc;
					} else if (bomEnc != null && bomEnc.startsWith("UTF-32")) {
						return bomEnc;
					} else {
						String msg = MessageFormat.format(
							"Invalid encoding, CT-MIME [{0}] CT-Enc [{1}] BOM [{2}] XML guess [{3}] XML prolog [{4}], encoding mismatch", cTMime, cTEnc, bomEnc, xmlGuessEnc, xmlEnc
					...
					String msg = MessageFormat.format(
						"Invalid encoding, CT-MIME [{0}] CT-Enc [{1}] BOM [{2}] XML guess [{3}] XML prolog [{4}], BOM must be NULL", cTMime, cTEnc, bomEnc, xmlGuessEnc, xmlEnc
					);
					throw new XmlStreamReaderException(msg, cTMime, cTEnc, bomEnc, xmlGuessEnc, xmlEnc);
				} else {
					return cTEnc;
	...
	private static String getXmlProlog(InputStream is, String guessedEnc) throws IOException {
		...
		if (guessedEnc != null) {
			...
			String xmlProlog;
			for (xmlProlog = ""; c != -1 && firstGT == -1 && offset < 4096; firstGT = xmlProlog.indexOf(62)) {
				offset += c;
				max -= c;
