...
public class DurationFormatUtils {
	...
	public static String formatDurationWords(long durationMillis, boolean suppressLeadingZeroElements, boolean suppressTrailingZeroElements) {
		...
		if (suppressTrailingZeroElements) {
			String tmpx = StringUtils.replaceOnce(duration, " 0 seconds", "");
			if (tmpx.length() != duration.length()) {
				duration = tmpx;
				tmpx = StringUtils.replaceOnce(tmpx, " 0 minutes", "");
				if (tmpx.length() != tmpx.length()) {
					duration = tmpx;
					tmpx = StringUtils.replaceOnce(tmpx, " 0 hours", "");
					if (tmpx.length() != tmpx.length()) {
						duration = StringUtils.replaceOnce(tmpx, " 0 days", "");
					}
				}
			}
		}
