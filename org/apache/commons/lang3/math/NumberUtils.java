...
public class NumberUtils {
	...
	public static boolean isCreatable(String str) {
		if (StringUtils.isEmpty(str)) {
			...
			int ix;
			for (ix = start; ix < sz || ix < sz + 1 && allowSigns && !foundDigit; ix++) {
			...
			} else if (chars[ix] >= '0' && chars[ix] <= '9') {
				return !SystemUtils.IS_JAVA_1_6 || !hasLeadingPlusSign || hasDecPoint;
			} else if (chars[ix] == 'e' || chars[ix] == 'E') {
				return false;
			} else if (chars[ix] == '.') {
				return !hasDecPoint && !hasExp ? foundDigit : false;
			} else if (!allowSigns && (chars[ix] == 'd' || chars[ix] == 'D' || chars[ix] == 'f' || chars[ix] == 'F')) {
				return foundDigit;
			} else {
				return chars[ix] != 'l' && chars[ix] != 'L' ? false : foundDigit && !hasExp && !hasDecPoint;
			}
		}
	}
