...
public class Conversion {
	...
	public static char binaryBeMsb0ToHexDigit(boolean[] src, int srcPos) {
		if (src.length == 0) {
			...
			int var5 = 0;
			if (paddedSrc[var5]) {
				if (paddedSrc.length > var5 + 1 && paddedSrc[var5 + 1]) {
					if (paddedSrc.length > var5 + 2 && paddedSrc[var5 + 2]) {
						return (char)(paddedSrc.length > var5 + 3 && paddedSrc[var5 + 3] ? 'f' : 'e');
					} else {
						return (char)(paddedSrc.length > var5 + 3 && paddedSrc[var5 + 3] ? 'd' : 'c');
					}
				} else if (paddedSrc.length > var5 + 2 && paddedSrc[var5 + 2]) {
					return (char)(paddedSrc.length > var5 + 3 && paddedSrc[var5 + 3] ? 'b' : 'a');
				} else {
					return (char)(paddedSrc.length > var5 + 3 && paddedSrc[var5 + 3] ? '9' : '8');
				}
			} else if (paddedSrc.length > var5 + 1 && paddedSrc[var5 + 1]) {
				if (paddedSrc.length > var5 + 2 && paddedSrc[var5 + 2]) {
					return (char)(paddedSrc.length > var5 + 3 && paddedSrc[var5 + 3] ? '7' : '6');
				} else {
					return (char)(paddedSrc.length > var5 + 3 && paddedSrc[var5 + 3] ? '5' : '4');
				}
			} else if (paddedSrc.length > var5 + 2 && paddedSrc[var5 + 2]) {
				return (char)(paddedSrc.length > var5 + 3 && paddedSrc[var5 + 3] ? '3' : '2');
			} else {
				return (char)(paddedSrc.length > var5 + 3 && paddedSrc[var5 + 3] ? '1' : '0');
			}
		}
	}
